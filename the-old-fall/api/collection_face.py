...
用法：
python collectingfaces.py --id 108 --imagedir C:/Users/my/Desktop/project/Recognition/the_old_fall/images
...
def collectionFaceAPI(id):
    audio_dir = r'C:\Users\my\Desktop\project\Recognition\the-old-facial-recogniton\audios'
    ...
    # 传入参数
    ap.add_argument("-ic", "--id", required=False, default=str(id),
                    help="")
    ap.add_argument("-id", "--imagedir", required=False,
                    default=r'C:\Users\my\Desktop\project\Recognition\the-old-fall\images_recog',
                    help="")
    ...
    while True:
        ...
        if error == 0 and face_count == 0:  # 没有检测到人脸
            ...
            audio_file = os.path.join(audio_dir, 'no_face_detected.mp3')
            # 去除路径中的引号
            ...
            audio_file = os.path.join(audio_dir, 'start_image_capturing.mp3')
            # 去除路径中的引号
            audio_file = audio_file.replace('"', '')
            playsound(audio_file)
            break
            ...
            audio_file = os.path.join(audio_dir, 'multi_faces_detected.mp3')
            # 去除路径中的引号
            audio_file = audio_file.replace('"', '')
            playsound(audio_file)
            error = 1
    ...
    for action in action_list:
        audio_file = os.path.join(audio_dir, action + '.mp3')
    ...
    # 结束
    print('[INFO] 采集完毕')
    audio_file = os.path.join(audio_dir, 'end_capturing.mp3')
    # 去除路径中的引号
    audio_file = audio_file.replace('"', '')
    playsound(audio_file)
