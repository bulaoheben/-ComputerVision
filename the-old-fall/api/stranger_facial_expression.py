...
class strangerFacialExpression():
    def __init__(self):
        ...
        self.facial_expression_model_path = '../models/face_expression.hdf5'
    ...
    def strangerFacialExpressionAPI(self, grabbed, frame, counter):
        ...
        for ((left, top, right, bottom), name) in zip(face_location_list,
            ...
            if 'Unknown' in names:  # alert
                if self.strangers_timing == 0:  # just start timing
                    ...
                    if difference < self.strangers_limit_time:
                        print('[INFO] %s, 房间, 陌生人仅出现 %.1f 秒. 忽略.' % (self.current_time, difference))
            ...
            draw.text((left, top - 30), final_label,
                      font=ImageFont.truetype(
                          r'C:\Users\my\Desktop\project\Recognition\the-old-facial-recogniton\NotoSansCJK-Black.ttc',
                          40),
                      fill=(255, 0, 0))  # linux
...
while True:
    ...
    (grabbed, frame, flag) = buf.strangerFacialExpressionAPI(grabbed, frame, counter)
    if flag:
        break
