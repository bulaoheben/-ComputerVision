...
class FaceUtil:
    ...
    def get_face_location(self, image):
        ...
        face_locations = face_recognition.face_locations(gray, number_of_times_to_upsample=1, model = self.detection_method)
        # 人脸位置
    ...
    def save_embeddings(self, image_paths, output_encoding_file_path):
        ...
        for (i, image_path) in enumerate(image_paths):
            ...
            name = image_path.split(os.path.sep)[-2] # folder name (person name)
            ...
            if len(encodings) != 1:
                ...
                warning += 'This file is deleted.\n'.format(len(encodings), str(image_path))
                continue
